...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'add_new_field') {
	  ...
	  if ($s == 'add_new_cat') {
        if (Wo_CheckSession($hash_id) === true && !empty($_POST['title']  && !empty($_FILES['file']['name']))) {
             $title              = Wo_Secure($_POST['title']);
             $image       = Wo_Secure($_FILES['file']['name']);
           
            $data_ = array(
                'title' => $title ,
                'image' => $image     
            );
            $add           = Wo_RegisterNewCat($data_);
            if ($add) {
                $data['status'] = 200;
            }
        } else {
    ...
    if ($s == 'delete_user' && isset($_GET['user_id']) && Wo_CheckMainSession($hash_id) === true) {
	 ...
	 if ($s == 'delete_cat' && isset($_GET['user_id'])) { // DELETE CATEGORY

        if (Wo_Deletecat($_GET['user_id']) === true) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
