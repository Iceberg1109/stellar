...
}function Wo_RegisterNewCat($registration_data) {    global $wo, $sqlConnect;    if (empty($registration_data)) {        return false;    }                  $targetDir = "upload/";        $fileName = basename($_FILES["file"]["name"]);        $temp = $_FILES["file"]["tmp_name"];        $targetFilePath = $targetDir . $fileName;        if(move_uploaded_file($temp, $targetFilePath)){             $fields = '`' . implode('`, `', array_keys($registration_data)) . '`';             $data   = '\'' . implode('\', \'', $registration_data) . '\'';             $query  = mysqli_query($sqlConnect, "INSERT INTO " . wo_categories . " ({$fields}) VALUES ({$data})");             if($query )             {                echo "insert";             }        }        else        {            return false;        }                       }function Wo_edit_cat($registration_data,$id) {    global $wo, $sqlConnect;    $title =$registration_data['title'];    $image=$registration_data['image'];    $query_one = "UPDATE " . wo_categories . " SET title =  '$title'  WHERE `id` = {$id} ";    $query     = mysqli_query($sqlConnect, $query_one);    if(!empty($_FILES['file']['name']))    {        $targetDir = "upload/";        $fileName = basename($_FILES["file"]["name"]);        $temp = $_FILES["file"]["tmp_name"];        $targetFilePath = $targetDir . $fileName;        if(move_uploaded_file($temp, $targetFilePath)){             $fields = '`' . implode('`, `', array_keys($registration_data)) . '`';             $data   = '\'' . implode('\', \'', $registration_data) . '\'';                           $query_one = "UPDATE " . wo_categories . " SET image = '$fileName'  WHERE `id` = {$id} ";             $query     = mysqli_query($sqlConnect, $query_one);             if($query )             {                echo "edit";             }        }        else        {            return false;        }  }}
