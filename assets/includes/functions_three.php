...
function Wo_GetCustomPage($page_name) {
    ...
    return $fetched_data;
}function Wo_User($page_name) {    global $sqlConnect;    if (empty($page_name)) {        return false;    }    $data          = array();    $page_name     = Wo_Secure($page_name);    $query_one     = "SELECT * FROM " . wo_categories . " WHERE `id` = '{$page_name}'";    $sql_query_one = mysqli_query($sqlConnect, $query_one);    $fetched_data  = mysqli_fetch_assoc($sql_query_one);    return $fetched_data;}
...
function wo_categories()
{
	 global $sqlConnect;
	  $data          = array();
	   $query_one     = "SELECT * FROM " . wo_categories;
	   $sql_query_one = mysqli_query($sqlConnect, $query_one);
	    while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
        $data[] = $fetched_data;
    }
...
function get_categories_user($id) {
    global $sqlConnect;
    $data          = array();
    $page_name     = Wo_Secure($page_name);
    // echo $query_one = "SELECT categories FROM " . wo_users . " where 'user_id' = 2";
    $query_one     = "SELECT categories FROM " . T_USERS. " WHERE `user_id` = $id";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
        $data[] = $fetched_data;
    }
